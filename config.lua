...
goal = {type="Strength", target=100000000, enabled=false}

hotkeys = {
 autoTrain=nil, autoRebirth=nil, rebirthLimit=nil, autoJungleRock=nil,
 autoEgg=nil, autoUltimates=nil, autoSize=nil, autoSpeed=nil,
 skipRebirthAnimation=nil, antiAFK=nil, esp=nil, coords=nil,
 automationOverlay=nil, performanceOverlay=nil, notifications=nil,
 autoPetNotifications=nil, autoAuraNotifications=nil
}
...
startingDurability = durability and (tonumber(durability.Value) or 0) or 0
character = nil
humanoid = nil
root = nil
gameGui = nil
...
notifications = {}
notificationFeed = {}
crystalStats = {opened=0, selectedPetHits=0, selectedAuraHits=0, rarityHits=0}
...
GlobalFunctions = nil
pcall(function()
 GlobalFunctions = require(ReplicatedStorage:WaitForChild("shared"):WaitForChild("modules"):WaitForChild("GlobalFunctions"))
end)

GUI_COLORS = {
 bg=Color3.fromRGB(18,18,21), panel=Color3.fromRGB(24,24,28), panel2=Color3.fromRGB(30,30,35),
 group=Color3.fromRGB(22,22,26), border=Color3.fromRGB(57,57,65), text=Color3.fromRGB(225,225,230),
 muted=Color3.fromRGB(150,150,158), blue=Color3.fromRGB(73,143,215), green=Color3.fromRGB(90,220,120),
 red=Color3.fromRGB(230,90,90), yellow=Color3.fromRGB(235,200,90), off=Color3.fromRGB(43,43,49),
 danger=Color3.fromRGB(95,40,44)
}
ESP_COLORS = {
 box=Color3.fromRGB(73,143,215), text=Color3.fromRGB(240,240,244),
 tracer=Color3.fromRGB(73,143,215), health=Color3.fromRGB(90,220,120)
}
FONT = Enum.Font.Code
...
function saveConfig()
 ...
 if type(writefile)=="function" then pcall(function() writefile(saveName,encoded) end) end
...
function loadConfig()
 ...
 if not data and type(isfile)=="function" and type(readfile)=="function" and isfile(saveName) then
  local ok,raw=pcall(readfile,saveName)
  if ok and raw then
   local good,decoded=pcall(HttpService.JSONDecode,HttpService,raw)
   if good then data=decoded end
  end
 end
 if type(data)~="table" then return end
 if type(data.state)=="table" then for k,v in pairs(data.state) do if state[k]~=nil then state[k]=v end end end
 if type(data.goal)=="table" then for k,v in pairs(data.goal) do if goal[k]~=nil then goal[k]=v end end end
 if type(data.hotkeys)=="table" then for k,v in pairs(data.hotkeys) do if hotkeys[k]~=nil then hotkeys[k]=v end end end
